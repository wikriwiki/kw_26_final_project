...
### 1. `target_dongs` 필드 추가 — 정밀 동 단위 적용

**문제**: 현재 contract는 `target_districts` (자치구 단위)만 지원. 적재 시 inject_json이 해당 자치구의 모든 동에 자동 펼침. 정책이 "강남역-역삼역 보행친화거리"처럼 *사업 구간*(특정 행정동 일대)인 경우 자치구 전체로 펼쳐져 의도와 어긋남 (P008 사례 — Cypher 수동 좁힘으로 정리).

**제안 스키마**:
```jsonc
{
  ...
  "target_districts": ["강남구"],               // 자치구 단위
  "target_dongs": ["역삼1동", "역삼2동", "도곡1동"],  // 신규 — 정밀 적용 시 사용
  ...
}
```

**적재 로직 변경**:
- `target_dongs` 있으면 → 해당 Dong 노드에만 `:applied_to` (자치구 전체 펼침 안 함)
- `target_dongs` 없고 `target_districts`만 있으면 → 자치구 + 그 자치구의 모든 Dong에 펼침 (현재 동작 유지)
- 둘 다 있으면 → 명시된 동 + 자치구

**LLM 추출 보강**:
- extractor 프롬프트에 "행정동명이 명시되었으면 target_dongs에, 자치구만 언급되었으면 target_districts에" 안내
- 동 enum (서울 423개 행정동) 검증을 validator에 추가

### 2. 지하철역·지구·랜드마크 단위 → 행정동 자동 매핑

**문제**: ".txt에 '강남역 인근', '테헤란로 일대' 등 비행정 명칭"을 LLM이 자치구로 일반화. 정확한 적용 동 추출 불가.

**제안**: 지하철역 → 위치 동 매핑 테이블(예: 강남역 → 역삼1동) + 도보권 동 확장 룰을 extractor에 주입. 또는 별도 후처리 단계.

### 3. 인지 범위 vs 직접 적용 범위 분리

**문제**: 현재 `applied_to` 하나가 정책 fetch 범위 + 직접 효과 적용 범위를 동시에 의미. 보행친화거리 같이 *물리적 사업 구간*과 *정책 인지 확산권*이 다른 경우 표현 불가.

**제안**:
- `:applied_to` — 직접 효과 적용 (사업 구간, 환급 적용 지역 등)
- `:awareness_zone` — 인지 확산권 (도보권, 인접 자치구 등) — 신규 관계 타입
- POLICY_CYPHER가 두 관계 모두 fetch하되 LLM 프롬프트엔 구분해서 노출

### 4. 발표일·시행일 갭의 시뮬 반영 (Awareness 모델)

**문제**: 현재 코드는 시행일~종료일만 효력 인지. 발표일~시행일 갭(시민 사전 인지·기대 효과 구간) 미반영.

**제안**: 메모리 §G의 lifecycle 6단계(S0~S5) + `policy_baseline` 트레잇 구현. agent별 awareness score를 동적 계산해 Dawn 컨텍스트에 노출. `FORMULAS.md` 의 "정책 수용" 섹션 참고.
