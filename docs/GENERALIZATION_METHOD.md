...
| 단일 런 보고 | **부트스트랩 CI · 반분 부호 일치 · 공통난수 쌍대 비교**(§8.1) |
| 단일 사례 | **정책 6개 + 홀드아웃 1개** |
| 기전과 느슨하게 연결된 결과 지표 | 기전 모듈별 파라미터 → 지표 대응을 명시 |

**두 번째 줄이 지금 계획을 바꿔야 할 두 번째 지점이다.** 다만 전 단계에서 시드를 바꿔
재실행하는 것은 예산상 불가능하다 — §8.1 로 대체한다.

### 8.1 복수 런 대신 — 잡음원을 나눠서 싸게 잡는다

시드를 바꾸는 것은 두 가지 잡음을 재려는 것이고, 둘은 비용이 전혀 다르다.

**① 에이전트 표본 잡음 — 재실행 없이 잡는다 (비용 0).**
한 런 안에서 에이전트를 복원추출해 지표별 **부트스트랩 신뢰구간**을 낸다. 추가로
표본을 반으로 갈라(75+75) **양쪽에서 부호가 같은지** 본다. n=150 규모에서는 이것이
잡음의 대부분이다.

**② LLM 샘플링 잡음 — 한 번만 재고 재사용한다 (비용: 런 1회).**
같은 에이전트·같은 날짜로 LLM 시드만 바꿔 **이중런을 딱 한 번** 돌려 집계 지표의
흔들림 폭을 측정하고, 그 분산성분을 이후 모든 런의 불확실성으로 쓴다.

> **"2시드 × 150명" 보다 "1런 × 300명" 이 낫다.** GPU 비용은 같은데 에이전트 표본
> 잡음 기준 정보량이 같고, 300명을 반분하면 부호 일치 검사도 똑같이 된다. 설정도
> 한 번이다. 시드를 늘릴 바에 사람을 늘린다.

복수 시드는 **최종 후보 1개와 홀드아웃에만** 적용한다. 비판적 리뷰가 문제 삼은 것도
런 횟수 자체가 아니라 **불확실성을 밝히지 않은 것**이므로, 부트스트랩 구간을 함께
보고하면 그 지적은 해소된다.

---

## 8.2 예산 (150명·1일 ≈ 16분 실측 기준)

| 항목 | GPU |
|---|---|
| 순차반감 3단계 (후보 거르기) | ~44시간 ≈ 2일 |
| LLM 잡음 이중런 1회 | 1.5시간 |
| 견고성 섭동 + 위약 검정 | ~8시간 |
| P012 본런 (2,000명 × 24일) | ~3.9일 |
| 홀드아웃 본런 | ~3.9일 |
| **합계** | **약 11일** |

배관 10일을 더해도 10월 초에 끝난다. 서버 회수는 11/05 17:00 이다.
...
6. **불확실성** — 거르는 단계는 부트스트랩 CI + 반분 부호 일치(비용 0), LLM 잡음은
   이중런 1회로 측정해 재사용. 복수 시드는 최종 후보·홀드아웃에만(§8.1).
