...
2. 시민별·날짜별 최종 `INCLUDES.actual_spent` 합계를 오프라인 실현 지출로, `State.online_spent`를 온라인 묶음 지출로 추출한다. 둘을 더한 **기록된 총지출 대리값**은 카드 결제수단을 구분하지 못한다. Stage1 의향이나 `cm_today_total_incl_online` 계획값을 실현 매출로 대체하지 않는다. 정책 사용가능업종은 실행에 동결한 정책 사본의 명시적 `eligibility`를 런타임과 채점기가 같은 범용 평가기로 적용해 다시 분류한다. 2025년 소비쿠폰의 `is_coupon_eligible`을 P013의 자로 전용하지 않는다.
...
2026-09-26 사용처 정합 수정: 실험 사본은 `data/experiments/P013_v53_policy_20260926.json`이다. 원본 P013의 지급액과 만료일은 그대로 두고, 등록한 5월 11일 관측 시작과 사용처 적격 규칙·표시만 명시했다. [행정안전부 2020-05-12 안내](https://www.mois.go.kr/video/bbs/type019/commonSelectBoardArticle.do?bbsId=BBSMSTR_000000000255&nttId=77107&searchCode1=A03)는 거주 광역시도 안 사용과 백화점·대형마트·온라인몰·유흥업소 제외를 설명한다. [행정안전부 담당자 설명](https://www.mois.go.kr/video/bbs/type019/commonSelectBoardArticle.do?bbsId=BBSMSTR_000000000255&nttId=77461&searchCode1=)은 직영점의 본사 소재지처럼 2025년 소비쿠폰과 다른 조건도 보여 준다. 따라서 기존 `is_coupon_eligible`로 P013 적격 매출을 채점한 값은 새 실험의 근거가 아니다. 사본의 규칙은 서울 POI에 대한 근사다. 실제 카드사 가맹 자격·직영점 본사·온라인 주문을 POI별로 관측하지 못하므로 직접 실측 비교의 잔여 한계로 기록한다. 새 사본은 정적 사전점검과 런타임/채점 규칙 일치 사례 검사를 통과했으나, 그래프에 적재하거나 P013 정책 팔을 실행하지는 않았다. 실제 런 전에 `--require-db`와 후보·원장 적격률을 다시 감사해야 한다.
