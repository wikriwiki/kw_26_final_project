...
with driver_session() as s, open(tmp, 'w', encoding='utf-8') as fh:
    rows = s.run("""
        MATCH (a:Agent)-[:HAS_PLAN]->(pl:Plan {day: date($dy)})-[i:INCLUDES]->(p:POI)
        ...
        RETURN pl.day_type AS day_type, i.category AS l1, i.sub_category AS sub,
               ...
               i.trigger AS trigger, i.pick_factor AS pick_factor,
               toString(i.time) AS time,
               // 공간 분석용 — 거주지 코드와 POI 코드는 같은 8자리 체계.
               // 직장 코드(workplace_dong_code_raw)는 7자리 외부 체계라 조인이 불가능하므로
               // 이름을 함께 내보낸다(거주지 이름-코드 쌍으로 사전을 만들어 대조).
               p.dong_code AS poi_dong,
               a.residence_dong_code_raw AS res_dong,
               a.residence_dong_name AS res_dong_name,
               a.workplace_dong_name AS work_dong_name""", dy=DAY)
    for r in rows:
        d = dict(r); d['day'] = DAY
