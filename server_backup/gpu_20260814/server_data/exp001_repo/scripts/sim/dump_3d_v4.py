...
출력: output/sim/dumps/3d_v4_midm/
"""
...
OUT_DIR = Path("G:/내 드라이브/Kw/final_project/output/sim/dumps/3d_v4_midm")
...
def dump_nodes(s, label: str, where: str = "", out_name: str = None, batch: int = 2000) -> int:
    """페이징 dump — heap OOM 방지를 위해 batch 단위로 fetch."""
    ...
    with open(out_path, "w", encoding="utf-8") as f:
        while True:
            q = f"MATCH (n:{label}) {where} RETURN n SKIP {skip} LIMIT {batch}"
            rows = list(s.run(q))
            if not rows:
                break
            for r in rows:
                ...
                n += 1
            skip += batch
            print(f"  {label}: {n:,} ({time.time()-t0:.1f}s)", flush=True)
    print(f"  {label}: {n:,} → {out_name} ({time.time()-t0:.1f}s)", flush=True)
    return n


def dump_includes(s) -> int:
    """Plan-INCLUDES-POI relation (시뮬 day만) — 페이징."""
        ...
        for day in DAYS:
            ...
            while True:
                ...
                for row in rows:
                    ...
                    n += 1
                skip += batch
            print(f"  INCLUDES {day}: cumulative {n:,} ({time.time()-t0:.1f}s)", flush=True)
    print(f"  INCLUDES (Plan→POI): {n:,} → includes.jsonl", flush=True)
    return n


def dump_conversation(s) -> int:
    """Conversation (시뮬 day만, 모든 properties) — 페이징."""
    ...
    with open(out_path, "w", encoding="utf-8") as f:
        for day in DAYS:
            skip = 0
            while True:
                q = f'MATCH (c:Conversation {{day: date("{day}")}}) RETURN c SKIP {skip} LIMIT {batch}'
                rows = list(s.run(q))
                if not rows:
                    break
                for r in rows:
                    ...
                    n += 1
                skip += batch
            print(f"  Conversation {day}: cumulative {n:,} ({time.time()-t0:.1f}s)", flush=True)
    print(f"  Conversation: {n:,} → conversation.jsonl", flush=True)
    return n


def dump_agent_summary(s) -> int:
    """Agent 페르소나 메타 (분석용 — 모든 필드) — 페이징."""
    ...
    with open(out_path, "w", encoding="utf-8") as f:
        while True:
            rows = list(s.run(f"MATCH (a:Agent) RETURN a SKIP {skip} LIMIT {batch}"))
            if not rows:
                break
            for r in rows:
                ...
                n += 1
            skip += batch
            print(f"  Agent: {n:,} ({time.time()-t0:.1f}s)", flush=True)
    print(f"  Agent: {n:,} → agents.jsonl", flush=True)
    return n
