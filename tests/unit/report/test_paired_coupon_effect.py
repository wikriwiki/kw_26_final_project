...
def test_p010_requires_independent_entitlement_and_complete_prepolicy_days():
    ...
    with pytest.raises(ValueError, match="issued amount mismatch"):
        score_coupon(on, off, roster=["a"], draws=0,
                     expected_amount_by_aid={"a": 201})
    with pytest.raises(ValueError, match="incomplete"):
        score_coupon(on[1:], off, roster=["a"], draws=0,
                     expected_amount_by_aid={"a": 200})
    with pytest.raises(ValueError, match="entitlement map"):
        score_coupon(on, off, roster=["a"], draws=0,
                     expected_amount_by_aid={"other": 200})
    on_two = on + [{**item, "aid": "b"} for item in on]
    off_two = off + [{**item, "aid": "b"} for item in off]
    with pytest.raises(ValueError, match="citizen grant receipts"):
        score_coupon(on_two, off_two, roster=["a", "b"], draws=0,
                     expected_amount_by_aid={"a": 150, "b": 250})
...
def test_p010_cli_rejects_manifest_from_modified_policy(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(sys, "argv", ["paired_coupon_effect.py", "--on", str(tmp_path / "on"),
                                     ...
                                     "--entitlements", str(tmp_path / "entitlements.json"),
                                     "--json-out", str(output)])
