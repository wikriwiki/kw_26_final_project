#!/usr/bin/env bash
# 범위의 산술 한 줄이 계획을 바꾸기는 하는가 — 24 호출. 사전등록 experiments/scope_fact/prereg.md
#
...
OUT=/data/scope_probe
LOG=$OUT/probe.log
mkdir -p $OUT
say(){ echo "[$(date +%F' '%H:%M:%S)] $*" | tee -a $LOG; }

say "맥락 생성 — 같은 시민 off/on 한 쌍씩"
...
import json, sys
seed = int(sys.argv[1])
rows = [json.loads(l) for l in open('/data/scope_probe/responses.jsonl', encoding='utf-8') if l.strip()]
keep = [r for r in rows if r.get('seed') == seed]
with open('/data/scope_probe/seed_%d.jsonl' % seed, 'w', encoding='utf-8') as fh:
    for r in keep:
