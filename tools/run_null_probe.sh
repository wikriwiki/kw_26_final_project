#!/usr/bin/env bash
# **영가설 대조** — 행동과 무관한 줄을 같은 자리에 끼워 같은 탐침을 돌린다. 48 호출. 사전등록 experiments/scope_fact/prereg.md
#
...
# 시드 둘. 한 시드만 쓰면 "달라졌다" 가 시드 잡음인지 그 줄 때문인지 못 가른다.
SEEDS = [1701, 2903]
...
import json, sys
seed = int(sys.argv[1])
rows = [json.loads(l) for l in open('/data/null_probe/responses.jsonl', encoding='utf-8') if l.strip()]
keep = [r for r in rows if r.get('seed') == seed]
with open('/data/null_probe/seed_%d.jsonl' % seed, 'w', encoding='utf-8') as fh:
    for r in keep:
        fh.write(json.dumps(r, ensure_ascii=False) + '\n')
