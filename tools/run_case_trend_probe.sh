#!/usr/bin/env bash
# 후보 2 — 확진 수준에 기준(2주 전 대비 배수)을 같이 주면 계획이 달라지는가. 48 호출. 사전등록 experiments/case_trend/design_note.md
#
...
import json, sys
seed = int(sys.argv[1])
rows = [json.loads(l) for l in open('/data/ct_probe/responses.jsonl', encoding='utf-8') if l.strip()]
keep = [r for r in rows if r.get('seed') == seed]
with open('/data/ct_probe/seed_%d.jsonl' % seed, 'w', encoding='utf-8') as fh:
    for r in keep:
        fh.write(json.dumps(r, ensure_ascii=False) + '\n')
