...
def main() -> int:
    ...
    for f in files:
        ...
        scored = sum(1 for r in d["results"] if r.get("hit") is not None)
        nul = [r for r in d["results"] if r.get("expect") == "0"
               and r.get("hit") is not None]
        agg[cand][pol] = [hits, scored, sum(1 for r in nul if r["hit"]), len(nul)]
        for r in d["results"]:
