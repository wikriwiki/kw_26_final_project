...
판정 우선순위:
  ...
  ② 상호명 룰 — 코드가 못 닿는 항목만  → arm 확정            (근거: brand_large)
  ...
  ②가 ③보다 앞인 이유: 코드가 '슈퍼마켓'이어도 상호가 "홈플러스…"면 대형업태로 잡아야 한다.
...
상호명으로 잡지 않는 것 (전용 코드도 없고 상호명도 안 통해 포기한 것):
  · 성인업소(성인용품점·안마방류) — G22199·S20802 잡화 통에 혼재. 상호명은 실측상
    복합어 매칭 0건 / '성인' 단독은 100% 오탐("삼성인쇄"·"성인피아노학원"). docs §4.3
  · 사행 중 카지노·경마·토토 — 서울에 내국인 카지노 부재. 실측 4건 중 3건 오탐. docs §9-8
  둘 다 제외군(C1 대조군)에 오탐을 넣는 손해가 포착 이득보다 크다는 같은 판단이다.
...
_CODE_VICE = {
    ...
    # 시행방안(2021-09-27 관계부처합동) 참고1: 사행업종 = "카지노, 복권방, 오락실 등".
    # 오락실이 명시돼 있어 제외로 돌린다. 이전에는 적립이었다 — 1차 자료 확인 전 판단.
    "R10404",   # 전자 게임장(오락실) 292
}
...
# =========================================================
# 사행(카지노·경마·토토)은 상호명 룰을 두지 않는다.
#   매핑 247코드에 전용 코드가 없어 한때 상호명 룰을 뒀으나, 원천 534,978건에
#   실측하니 4건 매칭 중 3건이 오탐이었다:
#       "카지노"(카페) · "카지노"(요리주점 I21104) · "복합리조트카지노인재개발연구소"(기타교육)
#   서울에는 내국인 출입 카지노가 없어(강원랜드뿐) 잡을 대상 자체가 존재하지 않는다.
#   그런데 이 룰의 결과는 excluded_vice = C1 대조군으로 들어가므로, 카페·학원이 섞이면
#   "제외군도 캐시백에 반응한다"는 가짜 신호가 된다. 성인업소를 뺀 것과 같은 판단이다.
#   → 사행 중 관측 가능한 것은 복권판매점(R10410, 코드로 정확히 포착)뿐이다. docs §9-8.
...
def sangsaeng_arm(
    ...
    # ② 상호명 룰 — 코드가 못 닿는 영역만 (현재는 대형업태 브랜드뿐)
    if n and _NAME_LARGE.search(n):
...
_CASES = [
    ...
    ("R10404", "○○게임장", "유원지·오락", ARM_EXCLUDED_VICE, "오락실 — 시행방안 참고1 사행업종"),
    ...
    ("G20404", "홈플러스 익스프레스 문래점", "슈퍼마켓", ARM_EXCLUDED_OTHER,
     "★ 코드는 슈퍼마켓인데 상호가 대형마트 — ②가 ③보다 앞"),
    # ── ★ 사행 상호명 룰 미도입 회귀 (원천 실측 오탐, docs §9-8) ──
    ("G20401", "카지노", "카페", ARM_ELIGIBLE, "★ 이름만 '카지노'인 카페 (실제 상호)"),
    ("I21104", "카지노", "일반주점", ARM_ELIGIBLE, "★ 코드는 요리주점 (실제 상호)"),
    ("P10613", "복합리조트카지노인재개발연구소", "기타교육", ARM_ELIGIBLE,
     "★ 교육기관 (실제 상호)"),
    # ── ② 상호명 룰 오탐 방지 ──
