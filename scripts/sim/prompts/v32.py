"""v25 with the resource check said to cover every activity, not only the given ones.

Given a meal stock of zero, the model planned two meals at home anyway - no groceries,
no eating out, no delivery - and the resource gate refused 46 of 96 cells. It handles
the detergent correctly, and the difference is that laundry is an injected fixed
activity while meals are ordinary ones it picks itself.

The instruction to check resources is already there. This only says it covers what the
model places, not just what it was handed. It is contract compliance, not a nudge:
refilling, going out, ordering in and skipping an optional meal all satisfy it.
"""
...
daily_conditions가 있으면 배치하는 모든 활동에 대해 필요한 자원을 실제 사용 전 확보할 수 있는지 확인한다.
