"""v25 with resource acquisition put into the order in which the plan is built.

The deliberation shows the model already understands the shortfall - "meal stock starts
at 0, but groceries gives 3 meals immediately" - and then loses it while laying out the
day around the fixed duty. Twenty-two of forty-eight zero-stock cells name groceries in
the reasoning and the plan still puts two free meals at home.

So the gap is not attention and not wording (v10) and not where the fact is written
(v11). It is that the ordering rule, which is what actually governs construction, says
nothing about resources. This adds them to it, and leaves both ways out open:
acquire the resource, or pick something that does not need it.
"""
