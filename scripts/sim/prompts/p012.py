# -*- coding: utf-8 -*-
"""P012 상생소비지원금 — 소비행동 프롬프트 (수정 가능)

현재는 p010 검증본과 바이트 단위로 같다. 캐시백 기전에 맞춰 조정이 필요하면
여기서 고친다 — P010 재현성에는 영향이 없다.
...
[정책·약속 반영]
- 정책 블록의 공통 사실과 개인별 적립 실적·문턱까지 남은 금액·남은 일수를 사실 그대로 읽는다.
...
{"time":"19:00","anchor":"zone:11680670","category":"쇼핑","sub_category":"가전·통신","intent":"드라이기 교체",
 "reasoning":"드라이기가 자꾸 꺼져 불편했지만 목돈이라 미뤄왔음. 문턱까지 얼마 안 남아 이번 달에 사면 캐시백까지 받겠다 싶어 퇴근길에 들름.","trigger":"policy"}
...
성향의 비율이다. 어제 컨디션, 개인 잔액, 적립 실적과 문턱, 일정, 실제 생활 필요를 함께 고려한다.
캐시백은 지금 쓸 돈을 늘려주지 않는다. 늘어나는 것은 돈이 아니라 **이번 달 안에 사면 일부를
돌려받는다는 조건**이다. 그래서 평소 같으면 다음 달로 미뤘을 일을 이번 달로 당길 이유가 생긴다.
미뤄둔 병원·치과, 바꿔야 할 가전, 아이 옷·신발처럼 언젠가는 할 일이 이 달 안으로 들어온다.
그런 날은 이벤트 수와 한 건당 지출이 함께 늘어난다.
문턱까지 남은 금액과 남은 일수를 본다. 평소 씀씀이로 자연히 넘길 상황이면 굳이 앞당길 이유가
없어 평소와 같다. 조금 모자라 이대로면 못 넘길 상황이고 당겨올 만한 일이 있다면 그날 씀씀이가
커진다. 이미 넘겼다면 더 쓸 유인이 사라져 다시 평소로 돌아간다.
얼마나 커지는지는 사람마다 다르다 — 미뤄둔 일이 얼마나 쌓였는지, 평소 씀씀이가 어땠는지,
통장에 여유가 있었는지에 달렸다. 다만 정책이 있다는 사실만으로 기계적으로 올리지는 않는다.
이번 달로 당겨올 실제 용건이 있을 때 올라간다.
단가와 결제수단은 Stage2가 결정하므로 여기서는 소비 의향만 표현한다.

[캐시백이 없어도 했을 지출의 몫 grant_kept_share — 최상위 필드]
오늘 계획한 지출을 하나씩 되짚어 본다. 그중 **캐시백 제도가 없었더라도 어차피 했을** 지출이
얼마나 되는지를 `grant_kept_share` (0~1)로 출력한다. 전부 어차피 했을 지출이면 1,
전부 문턱을 넘기려고 비로소 하게 된 지출이면 0이다.
끼니·장보기·생필품·약값처럼 매일 하던 지출은 캐시백과 무관하게 했을 것이다. 캐시백은 돈을
미리 주는 게 아니라 다음 달에 돌려받는 것이므로 오늘 쓸 수 있는 돈이 늘어난 것도 아니다.
반면 미뤄두던 옷·신발·가전이나 평소 아끼던 외식·나들이·미용을 **이번 달로 당겨** 했다면
그만큼은 문턱 때문에 생긴 지출이다.
문턱까지 남은 금액과 남은 일수를 함께 본다. 남은 금액이 평소 씀씀이로 자연히 채워질 정도라면
굳이 뭘 더 할 이유가 없어 1에 가깝다. 반대로 조금 모자라서 이대로면 못 넘길 상황이라면,
넘기기로 마음먹은 만큼이 새로 만든 지출이다. 이미 넘겼다면 더 쓸 유인이 사라져 다시 1에 가깝다.
문턱을 못 넘기면 이번 달 혜택은 0원이고, 넘긴 초과분의 10%만 돌아온다는 점도 감안한다.
오늘 실제로 계획한 지출 하나하나를 두고 판단한다.
