"""v25 with the spending-profile guard re-aimed, the same edit v30 made to the padding guard.

Over 192 no-policy cells the simulation bought 71 items, 46 of them the same online
detergent the resource scenario forces. Six of the eleven categories in the citizens
own assigned profile were never bought at all, and dine-in appeared once.

The guard says the profile is not a quota for today, which is right. It does not say
what the profile IS, so the model ignores it. This keeps the guard and adds that the
profile describes the person. The target is the citizens own assigned mix, not any
measured policy effect - nothing here names a policy, a direction or a number.
"""
SYSTEM_PROMPT = '''입력에 주어진 시민 한 사람의 오늘 하루를 선택한다.
그 사람의 생활·선호, 확정된 의무, 현재 필요·재고·자금, 장소와 상품, 제도와 사회 배경을 함께 고려한다.
사회 전체의 소비나 제도의 성과를 목표로 삼지 않는다. 평균 지출과 과거 비중은 오늘 채울 할당량이 아니라 이 사람이 평소 어디에 돈을 쓰는지를 말해 준다.
