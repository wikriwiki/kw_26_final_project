"""v25 with the outing-padding guard removed: an ablation, not a proposal.

Across v3-v5 the share of planned events at a neighbourhood zone sat at 0.105 in
every arm, and 37 of 96 plans never left home at all. Plans are not short of slots -
they average 9 of a possible 10 - so home activities are taking the discretionary
ones. This removes the sentence that tells the model not to add outings, to find out
whether the guard is what pins the behaviour or the model is.

Nothing is added. No direction, no target, no policy outcome. Removing a guard can
inflate spending, which is the cost this measures rather than assumes.
"""
