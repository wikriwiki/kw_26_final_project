"""v25 with the padding guard re-aimed at padding instead of at purchases.

v6 removed the guard entirely and the day did not go out any more than before
(zone share 0.1057 -> 0.1143, below the 0.03 line) but bought noticeably more
(purchase share 0.0571 -> 0.0750, 2.1 SE) with the activity count unchanged.
So the guard was holding down purchases, not outings.

Removing it is not the answer - it exists to stop padding. This keeps the guard
and takes the word "purchase" out of it, so it forbids filling a count without
naming purchases as the thing to avoid. One noun changes.
"""
