"""v25 + choosing WHERE an existing need is met.

The plans are almost entirely residence and workplace, so conditions attached to
neighbourhood places never reach the day. This adds the choice of place, and repeats
the guard against inventing needs so it cannot be read as a licence to spend.
No policy name, no desired sign, no measured target.
"""
...
채우기로 했다면 어디서 채울지도 고른다. 집·직장·동네가 각각 가능하고, 오늘의 조건이 그중 일부에만 걸리기도 한다.
