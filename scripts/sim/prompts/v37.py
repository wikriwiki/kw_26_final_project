"""v25 with the blanket permission to substitute one fulfillment for another removed.

Line 9 lets a need be met now, met from stock, or 대체·연기·생략 - substituted, deferred
or skipped. The plans take the substitution: a meal becomes home_delivery, a purchase
becomes home_online_goods, and the citizen's own card mix (마트 45%, 식사 30%) is met
without anyone going anywhere. Delivery was 19 of the 20 meal events in one arm.

Substitution is a real thing people do, so this is not obviously wrong - which is why it
is a candidate to TEST rather than a fix to adopt. Deleting the word leaves 연기·생략:
a need can still be put off or dropped. It does not say to go anywhere, name a place,
or name a category.

The risk this runs is the opposite of the last four rounds: purchases could FALL, because
the model drops needs it used to substitute. The pre-registration cancels the candidate if
that happens, on the same footing as the padding cancellation.
"""
