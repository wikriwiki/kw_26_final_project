...
OUT=/data/validation_v49/run
mkdir -p "$(dirname "$OUT")"
LOG=/data/validation_v49/v49.log
say(){ echo "[$(date +%F' '%H:%M:%S)] $*" | tee -a $LOG; }

say "사전등록 해시 대조"
sha256sum data/experiments/validation_v49.json scripts/sim/prompts/v48.py scripts/sim/prompts/v49.py | tee -a $LOG

say "768 호출 시작 (v45 · v49 각 384) — 재현"
...
/data/venv/bin/python scripts/sim/validate_prompt_v3.py \
  --config data/experiments/validation_v49.json --out "$OUT" 2>&1 | tee -a $LOG

say "=== 결과 ==="
/data/venv/bin/python - <<'PY' | tee -a $LOG
import json
d = json.load(open('/data/validation_v49/run/summary.json', encoding='utf-8'))
v = d['variants']
order = [n for n in ('v45', 'v49') if n in v]
...
rows = [json.loads(l) for l in open('/data/validation_v49/run/responses.jsonl', encoding='utf-8') if l.strip()]
...
print('  예측       방향이 유지되고 v49 가 v45 위, +5~+25칸. 안 되면 앞선 +20 은 런이었다')
print()
