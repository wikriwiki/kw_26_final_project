...
# 정책 JSON 선택 및 로드
# ═══════════════════════════════════════════════════════════════
_POLICIES_DIR = Path(__file__).resolve().parent.parent.parent / "data" / "neo4j_load" / "policies"


def select_policy_json() -> Path:
    """policies/ 디렉토리를 스캔해 사용자가 정책을 선택하도록 한다."""
    candidates = sorted(_POLICIES_DIR.glob("*.json"))
    if not candidates:
        print(f"[오류] {_POLICIES_DIR} 에 정책 JSON 파일이 없습니다.", file=sys.stderr)
        sys.exit(1)

    print("\n" + "=" * 60, file=sys.stderr)
    print("사용할 정책을 선택하세요", file=sys.stderr)
    print("=" * 60, file=sys.stderr)
    for i, path in enumerate(candidates, 1):
        try:
            meta = json.loads(path.read_text(encoding="utf-8"))
            label = f"{meta.get('id', path.stem)}  {meta.get('name', '')}  ({meta.get('type', '?')})"
        except Exception:
            label = path.name
    ...
    while True:
        try:
            ...
            if 1 <= idx <= len(candidates):
                ...
                print(f"\n→ 선택된 정책 파일: {chosen.name}\n", file=sys.stderr)
                return chosen
        except (ValueError, KeyboardInterrupt):
            pass
        print(f"  1~{len(candidates)} 사이 숫자를 입력하세요.", file=sys.stderr)


def load_policy_ctx(path: Path) -> dict:
    """정책 JSON을 읽어 보고서 전반에서 쓸 context dict를 반환한다."""
    ...
    return {
        ...
        "target_cats":     raw.get("benefit_categories", []),
        "target_districts": raw.get("target_districts", []),
        "income_grants":   raw.get("income_grants", {}),
        "excluded_income": raw.get("excluded_income", []),
        "effective_from":  raw.get("effective_from"),
        "effective_until": raw.get("effective_until"),
        "benefit_rate":    raw.get("benefit_rate"),
        "cap_per_agent":   raw.get("cap_per_agent"),
    }
...
# ═══════════════════════════════════════════════════════════════
_DISTRICT_NAME_TO_CODE = {
    "강남구": "11680", "서초구": "11650", "송파구": "11710", "강북구": "11305",
    "마포구": "11440", "용산구": "11170", "중구":   "11140", "종로구": "11110",
    "성동구": "11200", "광진구": "11215", "동대문구": "11230", "중랑구": "11260",
    "성북구": "11290", "도봉구": "11320", "노원구": "11350", "은평구": "11380",
...
def section2_before_after(start: date, days: int, policy_from: str, out_dir: Path, ctx: dict) -> tuple[dict, str]:
    cutoff = date.fromisoformat(policy_from)
    target_cats = ctx["target_cats"] or ["식사", "카페", "디저트"]

    # 대상 자치구 코드 결정 — ctx["target_districts"]에서 첫 번째 구 사용
    target_gu_name = next(
        (d for d in ctx["target_districts"] if d != "서울특별시"), None
    )
    target_gu_code = _DISTRICT_NAME_TO_CODE.get(target_gu_name or "", "11680")
    target_gu_label = target_gu_name or "강남"
    control_label = f"비{target_gu_label}"
    ...
    with driver_session() as s:
        pop = s.run("""
            MATCH (a:Agent)-[:LIVES_AT]->(:POI)-[:IN_DONG]->(:Dong)
              <-[:HAS_DONG]-(d:District)
            RETURN
              sum(CASE WHEN d.code = $code THEN 1 ELSE 0 END) AS gn,
              sum(CASE WHEN d.code <> $code THEN 1 ELSE 0 END) AS ng
        """, code=target_gu_code).single()
        GN_POP = int(pop["gn"]) or 1
        NG_POP = int(pop["ng"]) or 1
        print(f"  [pop] {target_gu_label} 거주 {GN_POP:,} / {control_label} 거주 {NG_POP:,}", file=sys.stderr)
        for i in range(days):
            ...
            gn = sum(r["spend"] for r in rows if r["dist"] == target_gu_code)
            ng = sum(r["spend"] for r in rows if r["dist"] != target_gu_code)
            daily.append({
                "day": d.isoformat(),
    ...
    ax.plot(xs, gn_per, marker="o", markersize=10, label=f"{target_gu_label} (n={GN_POP:,})",
            color="#e76f51", linewidth=3)
    ax.plot(xs, ng_per, marker="s", markersize=10, label=f"{control_label} (n={NG_POP:,})",
    ...
    if daily:
        ...
        fig, ax = plt.subplots(figsize=(13, 6.5))
        ax.plot(xs, gn_chg, marker="o", markersize=10, label=target_gu_label, color="#e76f51", linewidth=3)
        ax.plot(xs, ng_chg, marker="s", markersize=10, label=control_label, color="#4cc9f0", linewidth=3)
        ...
        ax.set_title(f"(C) DID — {target_gu_label} 변화율 − {control_label} 변화율 (정책 순효과)", pad=12)
...
# ═══════════════════════════════════════════════════════════════
def section3_spillover(start: date, days: int, out_dir: Path, ctx: dict) -> tuple[dict, str]:
    # 대상 자치구 + 인접 자치구 그룹을 ctx에서 구성
    target_gu_name = next(
        (d for d in ctx["target_districts"] if d != "서울특별시"), None
    )
    target_gu_code = _DISTRICT_NAME_TO_CODE.get(target_gu_name or "", "11680")
    target_gu_label = target_gu_name or "강남구"

    # 인접 자치구: 대상 지역 제외 후 고정 비교군 (강북은 원거리 대조군)
    _ADJACENT: dict[str, dict[str, str]] = {
        "11680": {"서초구": "11650", "송파구": "11710", "강북구": "11305"},
        "11650": {"강남구": "11680", "동작구": "11590", "강북구": "11305"},
        "11710": {"강남구": "11680", "강동구": "11740", "강북구": "11305"},
    }
    adjacent = _ADJACENT.get(target_gu_code, {"서초구": "11650", "송파구": "11710", "강북구": "11305"})
    groups = {target_gu_label: target_gu_code, **adjacent}
    target_cats = ctx["target_cats"] or ["식사", "카페", "디저트"]
    ...
    _palette = ["#e76f51", "#f4a261", "#f4d35e", "#8ecae6", "#a8dadc", "#457b9d"]
    colors = {name: _palette[i % len(_palette)] for i, name in enumerate(groups)}
    ...
    ax.set_title(f"자치구별 매출 추이 — {target_gu_label} 정책의 간접 영향 추적")
    ax.legend()
    ax.grid(alpha=0.3)
    plt.tight_layout()
...
# ═══════════════════════════════════════════════════════════════
# 섹션 2·3 dispatch — 정책 ctx에 따라 적절한 분석 함수 선택
# ═══════════════════════════════════════════════════════════════
def run_section2(ctx: dict, start: date, days: int, policy_from: str | None, out_dir: Path):
    """ctx 구조를 보고 섹션 2 분석 함수를 결정한다.

    - income_grants 있음  → 소득 군집별 DID (grant형)
    - target_cats 있음    → 지역×업종 before/after DID (facility·subsidy형)
    - 둘 다 없음          → None (분석 불가)
    """
    if not policy_from:
        return None
    if ctx["income_grants"]:
        print(f"  [dispatch] 섹션2 → 소득 군집별 DID ({ctx['id']})", file=sys.stderr)
        return section2b_income_did_p009(start, days, policy_from, out_dir, ctx)
    if ctx["target_cats"]:
        print(f"  [dispatch] 섹션2 → 지역×업종 before/after DID ({ctx['id']})", file=sys.stderr)
        return section2_before_after(start, days, policy_from, out_dir, ctx)
    print(f"  [dispatch] 섹션2 → 분석 불가 (income_grants·target_cats 모두 없음)", file=sys.stderr)
    return None


def run_section3(ctx: dict, start: date, days: int, out_dir: Path):
    """ctx 구조를 보고 섹션 3(spillover) 실행 여부를 결정한다.

    - 특정 자치구 대상 정책 → spillover 분석 실행
    - 전서울 대상('서울특별시'만 있음) → None (spillover 의미 없음)
    """
    non_city = [d for d in ctx["target_districts"] if d != "서울특별시"]
    if non_city:
        print(f"  [dispatch] 섹션3 → spillover 분석 ({', '.join(non_city)})", file=sys.stderr)
        return section3_spillover(start, days, out_dir, ctx)
    print(f"  [dispatch] 섹션3 → skip (전서울 대상 정책)", file=sys.stderr)
    return None
...
# ═══════════════════════════════════════════════════════════════
# 섹션 5 — 1대1 인터뷰 자동 (정책 ctx 기반 동적 구성)
# ═══════════════════════════════════════════════════════════════
...
# 정책 유형별 추가 질문 (Q1 다음에 삽입)
_QUESTIONS_BY_TYPE: dict[str, list[str]] = {
    "grant": [
        "지원금을 받고 나서 평소와 다르게 소비하신 게 있나요? 어떤 점이 달라졌나요?",
        "지원금 사용처를 고를 때 어떤 기준으로 결정하셨나요?",
    ],
    "facility": [
        "정비된 거리나 시설을 실제로 이용해 보셨나요? 이전과 비교해서 어떻게 느끼셨나요?",
        "주변 친구나 동료도 그 변화를 체감하고 있나요?",
    ],
    "subsidy": [
        "바우처·쿠폰을 사용할 때 어떤 가게를 선택하셨나요? 평소 가던 곳인가요, 새로 가본 곳인가요?",
        "할인 혜택이 없었다면 같은 가게에 갔을 것 같나요?",
    ],
    "regulation": [
        "규제 시행 후 평소 이용하시던 가게나 장소에 변화가 있었나요?",
        "규제가 일상에 불편함을 주거나 반대로 도움이 된 점이 있다면 말씀해주세요.",
    ],
    "tax": [
        "세제 혜택을 인지하고 계셨나요? 실제 소비 결정에 영향을 주었나요?",
        "혜택이 없었다면 소비 패턴이 달라졌을 것 같나요?",
    ],
    "transit": [
        "교통 환경 변화를 직접 체감하셨나요? 이동 경로나 빈도가 달라졌나요?",
        "교통 변화가 어떤 가게나 지역 방문에 영향을 주었나요?",
    ],
}


def _make_policy_question(ctx: dict) -> str:
    """Q1 — 정책 context에서 동적으로 생성."""
    name = ctx["name"] or ctx["id"]
    pid = ctx["id"]
    if ctx["income_grants"]:
        buckets = "·".join(f"{lv} {amt:,}원" for lv, amt in ctx["income_grants"].items())
        benefit = f"소득별 현금 지원금({buckets})"
    elif ctx["benefit_rate"]:
        cats = "·".join(ctx["target_cats"]) if ctx["target_cats"] else "전 업종"
        benefit = f"{int(ctx['benefit_rate'] * 100)}% 바우처({cats})"
    elif ctx["cap_per_agent"]:
        benefit = f"인당 {ctx['cap_per_agent']:,}원 지원"
    else:
        benefit = "정책 혜택"
    return (
        f"이번에 시행된 '{name}'({pid} 정책)에 대해 어떻게 느끼셨나요? "
        f"{benefit}을 직접 활용하셨다면 왜 그 가게에서 쓰셨고, "
        f"활용하지 않으셨다면 이유가 무엇인지 알려주세요."
    )


def _make_interview_questions(ctx: dict) -> list[str]:
    """ctx의 정책 유형에 따라 전체 인터뷰 질문 목록을 구성한다.

    구성: Q1(정책 질문) + 유형별 추가 질문(1~2개) + 공통 질문(5개)
    알 수 없는 type이면 공통 질문만 사용.
    """
    type_questions = _QUESTIONS_BY_TYPE.get(ctx["type"], [])
...
def _interview_sample_label(ctx: dict) -> str:
    ...
    grant → 지원금을 실제 사용한 에이전트(positive)
...
def build_markdown(start: date, days: int, policy_from: str | None,
                   ...
                   chart_dir_rel: str, ctx: dict | None = None) -> str:
    lines: list[str] = []
    pol_title = f" — {ctx['name']}" if ctx else ""
    lines.append(f"# 서울 상권정책 시뮬레이션 — 최종 보고서{pol_title}")
    lines.append("")
    ...
    if isinstance(s2b, dict) and "per_bucket" in s2b:
        ...
        pol_id = ctx["id"] if ctx else "정책"
        lines.append(f"## 2. 정책 효과 — 소득 군집별 차등 지급 비교 ({pol_id})")
        lines.append("")
        ...
        if ctx and ctx.get("income_grants"):
            ig = ctx["income_grants"]
            ig_desc = " > ".join(f"{lv} {amt//1000}k" for lv, amt in ig.items())
            lines.append(f"- {ctx['id']}는 소득별 차등 지급: {ig_desc}.")
        excl = ctx.get("excluded_income") if ctx else []
        if excl:
            lines.append(f"- 소득 '{'·'.join(excl)}'은 정책 비대상 → 분석에서 제외.")
        lines.append("")
    ...
    # 5) 인터뷰 — s5의 key가 ctx["type"] 기반으로 동적으로 결정됨
    pol_name = ctx["name"] if ctx else "정책"
    lines.append(f"## 5. 1대1 인터뷰 — {pol_name} 수혜 에이전트")
    lines.append("")
    for label, d in s5.items():
        if "error" in d:
            lines.append(f"### {label} — 샘플 없음")
            lines.append("")
            continue
        p = d["persona"]
...
def build_html(start: date, days: int, policy_from: str | None,
               ...
               s6: dict | None = None, ctx: dict | None = None) -> str:
...
# ═══════════════════════════════════════════════════════════════
def section2b_income_did_p009(start: date, days: int, policy_from: str, out_dir: Path, ctx: dict) -> dict:
    """소득 군집별 DID 분석. income_grants는 ctx에서 읽음.

    grant 받은 agent만 treatment 그룹 (jsonl grant_applied_today > 0).
    grant=0 agent (정책 적용 실패 + excluded_income) 모두 제외 → 정책 효과만 측정.
    """
    import json
    cutoff = date.fromisoformat(policy_from)
        ...
        with open(metrics_path, encoding='utf-8') as f:
            for line in f:
                try:
                    ...
                    pass
    print(f"  [{ctx['id']} 분석] 정책 적용 그룹: {len(grant_aids)}명", file=sys.stderr)

    grant_amounts: dict[str, int] = ctx["income_grants"]
    income_buckets: list[str] = list(grant_amounts.keys())
            ...
            if treatment_agents > 0:
                ...
                for r in rows:
                    try:
                        ...
                        aid = r['aid']
                        agent_spend[aid] = agent_spend.get(aid, 0) + int(sp_dict.get(ctx['id'], 0))
                    except Exception:
                        pass
                if agent_spend:
...
def main():
    ...
    ap.add_argument("--policy-json", default=None, help="정책 JSON 파일 경로 (생략 시 대화형 선택)")
    ...
    if args.policy_json:
        ...
        policy_path = select_policy_json()
    ctx = load_policy_ctx(policy_path)

    # --policy-from 미지정 시 JSON의 effective_from 사용
    ...
    print(f"[정책] {ctx['id']} — {ctx['name']}", file=sys.stderr)
    print(f"[1/7] 조건 요약 ...", file=sys.stderr)
    s1 = section1_conditions(start, args.days, policy_from)

    print(f"[2/7] 정책 효과 분석 ...", file=sys.stderr)
    s2 = run_section2(ctx, start, args.days, policy_from, chart_dir)

    print(f"[3/7] spillover 분석 ...", file=sys.stderr)
    s3 = run_section3(ctx, start, args.days, chart_dir)
    ...
    if args.skip_interview:
        ...
        print(f"[5/7] 인터뷰 ({ctx['name']} 수혜 1명 × 6 질문) ...", file=sys.stderr)
        s5 = section5_interviews(start, args.days, chart_dir, ctx)
    ...
    md = build_markdown(start, args.days, policy_from,
                        s1, s2, s3, s4_1, s4_2, s4_3, s5, chart_dir_rel, ctx=ctx)
    out_md.write_text(md, encoding="utf-8")

    print(f"[7/8] HTML 빌드 ...", file=sys.stderr)
    try:
        html_doc = build_html(start, args.days, policy_from,
                              s1, s2, s3, s4_1, s4_2, s4_3, s5, chart_dir, s6, ctx=ctx)
