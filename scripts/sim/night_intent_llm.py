...
1. 미래에 둘이 다시 만나거나 함께 무엇을 하자는 제안이 핵심이면 → "약속"
   - 예: "다음에 같이 가자", "언제 밥 먹자", "주말에 보자", "나중에 한번 보자"
   - 친밀도나 동선 겹침만으로 약속으로 분류하지는 않지만, 함께하자는 뉘앙스가 분명하면 약속을 우선한다.
   - 약속 날짜가 시뮬레이션 기간 밖이어도 여전히 "약속"이다.
2. 정책, 뉴스, 사건 전달이 핵심이면 → "이슈"
   - 단, 그 내용을 계기로 함께 만나자는 제안이 있으면 "약속"이 우선이다.
3. 장소나 행동을 권유하거나 좋았던 경험을 공유하며 가보라고 하면 → "추천"
   - 예: "거기 좋더라, 너도 가봐"
   - 상대가 각자 가보라는 뜻이면 추천이다. 함께 만나자는 뜻이 있으면 추천이 아니라 약속이다.
4. 그 외는 → "기타"
