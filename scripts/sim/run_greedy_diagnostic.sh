...
OUT=/data/validation_greedy/run
mkdir -p "$(dirname "$OUT")"
LOG=/data/validation_greedy/greedy.log
say(){ echo "[$(date +%F' '%H:%M:%S)] $*" | tee -a $LOG; }

say "사전등록 해시 대조"
sha256sum data/experiments/validation_greedy.json scripts/sim/prompts/v48.py scripts/sim/prompts/v49.py | tee -a $LOG

say "768 호출 시작 (v5 · v45 각 384) — temperature 0 진단"
...
/data/venv/bin/python scripts/sim/validate_prompt_v3.py \
  --config data/experiments/validation_greedy.json --out "$OUT" 2>&1 | tee -a $LOG

say "=== 결과 ==="
/data/venv/bin/python - <<'PY' | tee -a $LOG
import json
d = json.load(open('/data/validation_greedy/run/summary.json', encoding='utf-8'))
v = d['variants']
order = [n for n in ('v5', 'v45') if n in v]
...
rows = [json.loads(l) for l in open('/data/validation_greedy/run/responses.jsonl', encoding='utf-8') if l.strip()]
...
print('  예측       v45 가 91~95% 에 앉는다. 88%% 면 잔여는 전부 프롬프트, 97%% 면 전부 샘플러다')
print()
