...
OUT=/data/validation_v47/run
mkdir -p "$(dirname "$OUT")"
LOG=/data/validation_v47/v47.log
say(){ echo "[$(date +%F' '%H:%M:%S)] $*" | tee -a $LOG; }

say "사전등록 해시 대조"
sha256sum data/experiments/validation_v47.json scripts/sim/prompts/v47.py | tee -a $LOG

say "1152 호출 시작 (v5 · v45 · v47 각 384)"
...
/data/venv/bin/python scripts/sim/validate_prompt_v3.py \
  --config data/experiments/validation_v47.json --out "$OUT" 2>&1 | tee -a $LOG

say "=== 결과 ==="
/data/venv/bin/python - <<'PY' | tee -a $LOG
import json
d = json.load(open('/data/validation_v47/run/summary.json', encoding='utf-8'))
v = d['variants']
order = [n for n in ('v5', 'v45', 'v47') if n in v]
...
rows = [json.loads(l) for l in open('/data/validation_v47/run/responses.jsonl', encoding='utf-8') if l.strip()]
...
MDE = 21
for x, y in (('v5','v45'), ('v5','v47'), ('v45','v47')):
...
print('  주 지표    v5 → v47 이 0 을 제외한다')
print('  부 지표    v45 → v47 — 검출력 부족을 미리 등록했다')
print('  예측       v5 → v47 은 +35~+55칸 · v45 → v47 은 +10 근처이고 유의하지 않다')
print()
