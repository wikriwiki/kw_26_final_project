...
V2_CATALOG: tuple[AnalysisSpec, ...] = (
    AnalysisSpec("s1", "분석 개요", "run·정책·기간·처치군 정의를 고정합니다. 항상 포함됩니다."),
    AnalysisSpec("s2", "정책 사양", "시뮬레이션에 배선된 정책 JSON 의 내용과 지급 구간을 표로 싣습니다."),
    AnalysisSpec("s3", "소비 총량 추이", "일자별 총액·건수·객단가 시계열."),
    AnalysisSpec("s4", "시행 전후 겹쳐보기", "같은 길이의 전/후 구간을 한 축에 겹쳐 차이를 면으로 보여줍니다."),
    AnalysisSpec("s5", "업종별 전후 비교", "업종별 일평균 전후 비교와 증감률."),
    AnalysisSpec("s6", "이중차분 (DID)", "정책 대상 업종을 처치군, 비대상 업종을 대조군으로 둔 2×2 이중차분."),
    AnalysisSpec("s7", "업종별 이중차분", "어떤 업종에서 정책 때문에 금액이 늘었는지 업종별 DID."),
    AnalysisSpec("s8", "분위별 효과", "metrics 의 spend_decile 기준 1인당 소비 변화."),
    AnalysisSpec("s9", "소비 구조", "결제 구성·요일 유형·업종 위치·지역 분포."),
    AnalysisSpec("s10", "일관성 검증", "보고서 안의 항등식을 다시 계산해 대조합니다. 항상 포함됩니다."),
    AnalysisSpec("s11", "근거와 한계", "출처·해설 생성 방식·해석 한계. 항상 포함됩니다."),
)
