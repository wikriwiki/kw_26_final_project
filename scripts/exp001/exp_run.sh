...
for i in $(seq 1 60); do $NEO/bin/cypher-shell -a bolt://localhost:7687 -u neo4j -p "$NEO4J_PASSWORD" 'RETURN 1' >/dev/null 2>&1 && break; sleep 3; done

export NEO4J_URI=bolt://localhost:7687 NEO4J_USER=neo4j NEO4J_PASSWORD=${NEO4J_PASSWORD:-changeme} NEO4J_DATABASE=neo4j PYTHONIOENCODING=utf-8
