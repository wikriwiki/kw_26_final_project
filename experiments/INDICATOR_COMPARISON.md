...
```
         ...
         감소                             증가
P012-1                     │█████               실측 +20.8
                           │██████████████████  시뮬 +79.2  [-79.2, +980.0]  ← 0을 지난다

P012-2                     │█                   실측 +2.9
                           │████████            시뮬 +34.2  [-32.8, +171.9]  ← 0을 지난다
...
|---|---|:-:|---:|---:|---|---|---|
| P012-1 | 적립업종 지출 (개인 내 쌍체차) | `+` | +20.8 | +79.2 | [-79.2, +980.0] **0 포함** | 부호 일치 *(구간이 0을 지나 단정 못 함)* | **3.8배** |
| P012-2 | 제외업종 무반응 — 방어선 | `0` | +2.9 | +34.2 | [-32.8, +171.9] **0 포함** | 방어선 — 아래 주석 | — |
...
| EM-3 | 전체 카드매출 증가 (+7.3%) | `+` | +7.3 | +127.7 | [+19.4, +377.8] | 부호 일치 | **17.5배** |
...
```
         ...
         감소                             증가
LV-1                       │                    실측 +0.0
                           │██████████████████  시뮬 +36.2  [-42.0, +194.1]  ← 0을 지난다

LV-2                                            실측 수치 없음
                          █│                    시뮬 -2.3  [-18.5, +10.1]  ← 0을 지난다
...
| 지표 | 무엇을 재나 | 기대 | 실측 | 시뮬(합산) | 95% 구간 | 부호 | 크기 |
|---|---|:-:|---:|---:|---|---|---|
| LV-1 | 소비지출 규모 무반응 — 방어선 | `0` | +0.0 | +36.2 | [-42.0, +194.1] **0 포함** | 방어선 — 아래 주석 | — |
| LV-2 | 거주 행정동 소비 비중 증가 | `+` | 수치 없음 | -2.3 | [-18.5, +10.1] **0 포함** | **부호 반대** | — |
...
```
         ...
         감소                             증가
DS-1                     ██│                    실측 -14.1
                           │██████████████████  시뮬 +138.8  [-45.5, +1300.0]  ← 0을 지난다

DS-2                       │█                   실측 +4.2
                           │██                  시뮬 +13.4  [-47.5, +143.4]  ← 0을 지난다

DS-3                       │██                  실측 +18.3
           ████████████████│                    시뮬 -125.4  [-1298.2, +111.5]  ← 0을 지난다
...
| 지표 | 무엇을 재나 | 기대 | 실측 | 시뮬(합산) | 95% 구간 | 부호 | 크기 |
|---|---|:-:|---:|---:|---|---|---|
| DS-1 | 음식점 지출 감소 (−14.1%) | `-` | -14.1 | +138.8 | [-45.5, +1300.0] **0 포함** | **부호 반대** | **9.8배** |
| DS-2 | 소매업 지출 증가 (+4.2%) | `+` | +4.2 | +13.4 | [-47.5, +143.4] **0 포함** | 부호 일치 *(구간이 0을 지나 단정 못 함)* | **3.2배** |
| DS-3 | 소매 > 음식점 (+4.2 − (−14.1)) | `rank` | +18.3 | -125.4 | [-1298.2, +111.5] **0 포함** | **순위 반대** | **6.9배** |
...
- **부호를 말할 수 있는 지표는 1개뿐이다** — EM-3. **그리고 그 하나도 불안정하다.**
  블록 집합을 바꿔 재면 이렇다.

  ```
  옛 4런 (v3·v4·v5·v6)   EM-3 +127.7  [+19.4, +377.8]   0 제외
  v7 블록 (63001~4)       EM-3  +74.8  [ -7.1, +262.7]   0 포함
  v8 블록 (64001~4)       EM-3  +57.9  [-19.0, +239.6]   0 포함
  ```

  **세 번 중 한 번만 0 을 제외한다.** 그러므로 "부호를 말할 수 있다"도 강하게 읽으면 안 된다.
  구간은 **블록(seed 묶음)을 먼저 재추출하고 그 안에서 칸을 재추출**해 낸 것이다 —
  칸만 재추출하던 이전 방식은 구간이 너무 좁았다.
